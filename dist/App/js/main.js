...
Chat.prototype = {
    ...
    init: function init() {
        ...
        reg = /^[\w\u4e00-\u9fa5]*$/,
            //用户名匹配正则
        login_input = $('input.login_input').focus(),
            //用户名输入框
        login_info = $('div.login_info'),
            //登陆信息框
        msgArea = $('textarea.msg'),
            //信息输入框
        bgTotalNum = 6,
            //背景图总数
        boxList = [$('ul.font_size_box'), //字号弹窗
        $('ul.font_family_box'), //字体弹窗
        $('ul.font_color_box'), //颜色弹窗
        $('div.emoji_box') //表情弹窗
        ],
            btnList = [$('span.sprite-size'), //字号按钮
        $('span.sprite-family'), //字体按钮
        $('span.sprite-color'), //颜色按钮
        $('span.sprite-emoji') //表情按钮
        ],
            audioList = [$('audio.audio_msg'), $('audio.audio_global'), $('audio.audio_sys')];
        //初始化变量
        var that = this,
            userName = '',
            //用户名
        ...
        var _loop = function _loop(i) {
            var styleList = ['font_size', 'font_family', 'color'];
    ...
    //    初始化弹窗
    _initialPopup: function _initialPopup() {
        //Emoji
        var emojiBox = $('div.emoji_box'),
        ...
        emojiBox.html(html);
        //color
        for (var _i = 0; _i < colorLiList.length; _i++) {
            var li = colorLiList[_i];
            $(li).css({ 'background-color': li.innerHTML, 'color': li.innerHTML });
        }
        //font-family
        for (var _i2 = 0; _i2 < famLiList.length; _i2++) {
            var _li = famLiList[_i2];
            $(_li).css({ 'font-family': _li.innerHTML });
        }
        //font-size
        for (var _i3 = 0; _i3 < sizeLiList.length; _i3++) {
            var _li2 = sizeLiList[_i3];
            $(_li2).css({ 'font-size': _li2.innerHTML });
        }
    },
    //    显示新消息
    _displayNewMsg: function _displayNewMsg(user, msg, font, img) {
        var container = $('.article'),
            userColor = '',
    ...
    //    更新用户列表
    _displayUsers: function _displayUsers(users, me) {
    ...
    //    显示表情
    _showEmoji: function _showEmoji(msg) {
        var match = void 0,
            result = msg,
            reg = /\[emoji:\d+\]/g,
            emojiIndex = void 0,
